...
中转服务商存在额度重置、限流、宕机等情况。网关自动检测故障并切换到下一个可用服务商，定期自动切回，无需手动干预。

**多账号切换繁琐**

中转服务商和官方账号都支持拖拽排序、优先级切换。模型名称根据映射规则自动转换，不支持的模型自动跳过。

**多 CLI 重复配置**

MCP Server 、预设提示词、Skill、插件等工具一次配置，可快速应用到多个 CLI。
...
- 拖拽快速调整优先级
- 模型映射：服务商模型名称与 CLI 模型名称不一致时自动映射，支持通配符匹配
  - `*` 匹配任意长度字符，`?` 匹配单个字符。
...
- **MCP Server**：一键为多个 CLI 启用/禁用
- **预设提示词**：一键为多个 CLI 切换全局提示词
- **Skill**：可视化管理
- **插件**：可视化管理
...
于是产生了一个可以作为机制的 bug：在现有服务商中点击编辑，再将弹窗关闭。切换到另外一个 CLI 的配置 Tab，点击添加服务商。发现原本的内容居然还在，直接保存就好啦。
