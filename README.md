...
- 🔀 **故障转移** - 上游失败自动重试、切换服务商、阶梯熔断冷却，用户零感知
...
- 支持按日期区间与快捷区间筛选
- 统计数据定时自动刷新，可随时暂停。
- 网关设置：点击标题旁的状态点可打开配置弹窗，集中修改监听地址、监听端口、数据目录、文件日志和日志级别。
- 对外访问：监听 `0.0.0.0`、`::` 或局域网 IP 后，其他设备可通过本机局域网 IP 访问。
...
- 故障转移规则：渠道失败（超时、5xx、429 等）在网关内重试，每次失败计入连续失败计数；计数命中熔断档位（可配置多档递进，默认连续 5 次拉黑 10 分钟）后服务商熔断冷却并切换下一个服务商。失败计数跨冷却期保留，熔断到期自动放行试探流量，成功一次即清零，反复故障会命中更高档、冷却更久。401/403/404（密钥错误、模型不存在等）不会自愈，直接按最小档熔断；400/413/422（请求本身错误）直接返回客户端，不重试不熔断。
...
- 清理粒度：全部日志 / 全部详情 / 统计数据 / 30 天前日志 / 30 天前详情。
...
CCG Gateway 支持通过环境变量或网关设置弹窗进行配置，所有配置项均有默认值，开箱即用。
...
配置优先级为「环境变量 > 网关设置弹窗 > 默认值」。存在有效的环境变量时，弹窗中对应的输入框会被禁用，避免配置被意外覆盖。
...
> macOS 从 Finder 启动的安装版通常不会读取 Shell 配置文件中的环境变量；安装版建议通过标题旁的网关状态点打开设置弹窗。
