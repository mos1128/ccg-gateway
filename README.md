...
CCG Gateway 是一款面向 Claude Code、Codex、Gemini CLI、OpenCode、Kimi Code、ZCode 等 Agent 的桌面端管理工具，集智能网关、账号管理、配置管理于一体。
...
服务商出现额度重置窗口期、限流、宕机等情况时，网关自动切换至可用服务商，定期回检，用户零感知。

同时支持：服务商可用性检测；模型名称映射；模型缺失自动路由至其他服务商；自定义请求的 User-Agent。
...
统计报表涵盖了服务商 / 模型双维度的 Token 用量、费用明细、请求数量。

**请求信息不透明**

请求日志记录了每次调用的状态、耗时、Token 计量、费用、Agent 请求、服务商响应等数据，信息一目了然。
...
- 提供服务商 / 模型双维度的统计，涵盖 Token 用量 / 请求次数 / 费用。
- 可在日志管理中清理历史统计数据。
...
- 内置支持 Claude Code、Codex、Gemini CLI、OpenCode、Kimi Code 和 ZCode。
...
- 费用配置：可分别配置输入、输出、缓存读取、缓存创建的每百万 Token 单价，供统计和日志自动计算费用。
...
- 请求日志：分为请求元数据与请求详情
  - 元数据：请求时间、Agent、服务商、状态、耗时、Token 明细、费用、模型映射、错误信息等。
