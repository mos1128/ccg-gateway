...
所有顶层字段和 `features` 下的功能都必须出现。暂不支持的功能只写 `{ "enabled": false }`。
...
该功能只管理凭证文件和认证字段，不执行登录命令、OAuth 或 Token 刷新。
...
- `replace_file`：用凭证中的完整 JSON 替换目标文件。
...
| `format` | 目标格式 | 仅 `set_field` 使用，固定为 `json`。 |
