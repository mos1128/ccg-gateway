<template>
  <div class="discover-item">
    <div class="discover-info">
      ...
      <el-tooltip v-if="plugin.description" effect="light" placement="top" :enterable="true" :show-after="200">
        <template #content>
          <div class="text-14" style="max-width: 350px; line-height: 1.6; word-break: break-word; user-select: text; color: var(--color-text-dark);">
            {{ plugin.description }}
          </div>
        </template>
        <div class="discover-desc" @click="$emit('copy-description', plugin.description)">
          {{ plugin.description }}
        </div>
      </el-tooltip>
