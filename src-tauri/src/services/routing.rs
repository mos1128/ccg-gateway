...
pub async fn get_passthrough_provider(
    ...
    let candidates: Vec<(Provider, Protocol)> =
        get_enabled_configured_providers(db, cli_type, profile)
            .await?
            .into_iter()
            .filter_map(|provider| {
...
pub async fn get_available_providers(
    ...
    let providers = get_enabled_configured_providers(db, cli_type, profile).await?;
